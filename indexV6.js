  ...
  return fetch(url)
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        results.push(result);
        links.push(data.items[i].displayLink);
        break;
      }
    })
    .catch((error) => {
...
Promise.all(promises)
  .then(async () => {
    ...
    try {
      const linksData = await fs.readFile("links.json", "utf8");
    ...
    const updatedLinks = existingLinks.concat(links);

    const linksJSON = JSON.stringify(updatedLinks, null, 2);
    await fs.writeFile("links.json", linksJSON, "utf8");
    console.log(
      'Links successfully saved in "links.json" file.'
    );
  })
  .catch((error) => {
